...
case ${TEST_SUITE} in
    ...
    checkGitSubmodule)
        ...
        ;;
    checkGruntClean)
        COMMAND="find 'typo3/sysext' -name '*.js' ! -path '*/Fixtures/*' ! -path '*/ContentBlocks/*' -exec rm '{}' + && cd Build; npm ci || exit 1; node_modules/grunt/bin/grunt build; cd ..; git add *; git status; git status | grep -q \"nothing to commit, working tree clean\""
