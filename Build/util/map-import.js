...
const mapImport = (targetModule, context) => {
  ...
  if (targetModule.charAt(0) === '.') {
    targetModule = path
      ...
      .replace(/^.*\/Build\/JavaScript\/([^\/]+?)\/Resources\/Public\/TypeScript/, (match, extname) => '@typo3/' + ('_' + extname).replace(/_./g, (e) => e.charAt(1).toUpperCase()));
  }
    ...
    imports.map(i => {
      if (i.d === -2) {
        ...
        if (importExpr.match(/^['"][^'"]+['"]$/)) {
          ...
          console.log('dynamic', importValue, mappedValue);
        ...
        const mappedValue = mapImport(importValue, srcpath);
        console.log('static', importValue);
        if (mappedValue !== importValue) {
