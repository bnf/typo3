...
interface Configuration {
  ...
  content: TemplateResult | string | JQuery;
...
export class ModalElement extends LitElement {
  ...
  @state() templateResultContent: TemplateResult | JQuery = null;
  ...
  protected render(): TemplateResult {
    ...
    return html`
      <div tabindex="-1" class="modal fade t3js-modal ${classMap(classes)}" style=${styleMap(styles)}>
  ...
  private renderModalBody(): TemplateResult | JQuery {
...
class Modal {
  ...
  private static resolveEventNameTargetElement(evt: Event): HTMLElement | null {
    ...
    if (target && target.dataset && target.dataset.eventName) {
      return target;
    } else if (currentTarget && currentTarget.dataset && currentTarget.dataset.eventName) {
  ...
  private initializeMarkupTrigger(theDocument: HTMLDocument): void {
    const modalTrigger = (evt: Event, triggerElement: HTMLElement): void => {
      ...
      const severity: SeverityEnum = typeof SeverityEnum[<any>triggerElement.dataset.severity] !== 'undefined'
        ? SeverityEnum[<any>triggerElement.dataset.severity] as any
        : SeverityEnum.info;
  ...
  private generate(configuration: Configuration): ModalElement {
    ...
    currentModal.addEventListener('show.bs.modal', (): void => {
