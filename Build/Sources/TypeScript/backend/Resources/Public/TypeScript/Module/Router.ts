...
export class ModuleRouter extends LitElement {
  ...
  constructor() {
    ...
    this.addEventListener('typo3-module-load', (e: CustomEvent) => {
      ...
      if (tagName === 'typo3-iframe-module') {
        ...
        if (url.includes(this.getAttribute('state-tracker') + '?state=')) {
          ...
          if (slot && slot.getAttribute('name') !== state.tagName) {
            ...
            if (moduleElement) {
              moduleElement.setAttribute('endpoint', state.detail.url);
            }
          }
  ...
  public render(): TemplateResult {
    ...
    element.setAttribute('endpoint', this.endpoint);
  ...
  private _handlePopstate(event: PopStateEvent) {
    ...
    if (event.state.url) {
      this.setAttribute('endpoint', event.state.url);
      this.decorate = true;
    } else {
