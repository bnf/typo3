...
class ContextMenu {
  ...
  private populateData(items: MenuItems, level: number): void {
    ...
    this.element.updateComplete.then(() => {
      const firstItem = <HTMLElement> this.element.renderRoot.querySelector('li[role="menuitem"][tabindex="-1"]')
      if (firstItem) {
        firstItem.focus();
      }
    });
...
class ContextMenuElement extends LitElement {
  ...
  @property({type: Number, reflect: true}) submenu: number = null;
  ...
  private openSubmenu(level: number, item: HTMLElement): void {
    ...
    this.submenu = parseInt(item.dataset.index, 10);
  }
  ...
  private drawMenu(items: MenuItems, level: number): TemplateResult {
    const renderItem = (item: MenuItem, index: number) => {
      if (item.type === 'item') {
        ...
        return html`
          <li role="menuitem" aria-haspopup="true" class="submenu" tabindex="-1" data-index="${index}" @click="${this.handleListGroupItemClick}" @keydown="${this.handleListGroupItemKeydown}">
            ${item.label}&nbsp;&nbsp;<typo3-backend-icon identifier="actions-chevron-right" size="small"></typo3-backend-icon>
            ${this.submenu !== index ? '' : html`
