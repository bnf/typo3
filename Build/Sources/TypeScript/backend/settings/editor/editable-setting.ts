...
export class EditableSettingElement extends LitElement {
  ...
  protected render(): TemplateResult {
    const { value, valueFromSets, definition } = this.setting;
    return html`
      <div
        class=${`settings-item settings-item-${definition.type}`}
        id=${`setting-${definition.key}`}
        tabindex="0"
        data-status=${value === valueFromSets ? 'none' : 'modified'}
      >
  ...
  protected setToDefaultValue(): void {
    if (this.typeElement) {
      this.typeElement.value = this.setting.valueFromSets as unknown;
    }
  }
