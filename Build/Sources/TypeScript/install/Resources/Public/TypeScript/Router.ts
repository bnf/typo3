...
class Router {
  ...
  public initialize(): void {
    ...
    $(document).on('click', '.card .btn', (e: JQueryEventObject): void => {
      ...
      if (isInline) {
        import(requireModule + '.esm.js').then(({default: aModule}: {default: AbstractInlineModule}): void => {
        ...
        const $modal = Modal.advanced({
          ...
          callback: (currentModal: any): void => {
            import(requireModule + '.esm.js').then(({default: aModule}: {default: AbstractInteractableModule}): void => {
