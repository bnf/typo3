...
export class Dashboard extends LitElement {
  ...
  constructor() {
    ...
    this.addEventListener(DashboardEditEvent.eventName, async (event): Promise<void> => {
      ...
      try {
        const response = await action('/dashboards/{dashboardIdentifier}', { dashboardIdentifier: identifier }).patch({ title }, asJson);
        const data = await response.resolve();
    ...
    this.addEventListener(DashboardUpdateEvent.eventName, (event): void => {
      ...
      action('/dashboards/{dashboardIdentifier}/widgetPositions', { dashboardIdentifier: identifier })
        .put({
          widgets,
          widgetPositions,
        }, asJson)
    ...
    this.addEventListener(DashboardDeleteEvent.eventName, (event): void => {
      event.preventDefault();
      const { identifier } = event;
      action('/dashboards/{dashboardIdentifier}', { dashboardIdentifier: identifier })
        .delete()
        .then(async (): Promise<void> => {
  ...
  private async addWidget(): Promise<void> {
    ...
    wizard.addEventListener(newRecordWizardEventName, async (event): Promise<void> => {
      ...
      try {
        const response = await action('/dashboards/{dashboardIdentifier}/widgets', { dashboardIdentifier: this.currentDashboard.identifier })
...
export class DashboardWidget extends LitElement {
  ...
  private readonly fetchTask = new Task(this, {
    ...
    task: async ([widgetIdentifier, dashboardIdentifier], { signal }): Promise<DashboardWidgetInterface> => {
      const response = await action('/dashboards/{dashboardIdentifier}/widgets/{widgetIdentifier}', {
  ...
  private async editSettings(): Promise<void> {
    ...
    try {
      const response = await action('/dashboards/{dashboardIdentifier}/widgets/{widgetIdentifier}/settings', {
