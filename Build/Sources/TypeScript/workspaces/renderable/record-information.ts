...
export class RecordInformationElement extends LitElement {
  ...
  protected override render() {
    return html`
      <div>
        <p>${labels.render('path', { '0': html`${this.record.path_Live}`, b })}</p>
        <p>${labels.render('current_step', { '0': html`${this.record.label_Stage}`, '1': this.record.stage_position, '2': this.record.stage_count, b: chunks => html`<strong data-itworks="yeah2">${chunks}</strong>` })}</p>
;
