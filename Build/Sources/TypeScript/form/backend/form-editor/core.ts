...
type FinisherDefinition = {
  label: string
  iconIdentifier?: string,
  // @todo should this be a generic/base type of all FormEditorDefinitions-sub-sub-childs?
  predefinedDefaults: object,
}
...
type ModelMetadata<T extends object> = {
  __parentRenderable: FormElement,
  __identifierPath: string,
  renderables: boolean | null | Model<T, ModelMetadata<T>>
};
 ...
 */
function extendModel<D extends object, T extends ModelMetadata<D>>(
...
export class Model<D extends object, T extends ModelMetadata<D>> {
   ...
   * @publish mixed
   */
  public set(key: string, value: any, disablePublishersOnSet?: boolean): void {
    let path: string;
    ...
    while (path.indexOf('.') > 0) {
      ...
      // is converted to an object when a non-numeric path segment is found
        ...
        obj[firstPartOfPath] = obj[firstPartOfPath].reduce(
          // @todo(bnf) get rid of any
          function(converted: any, item: any, itemIndex: any): any {
  ...
  public toString(): string {
    ...
    if (!utility.isUndefinedOrNull(objectData.__parentRenderable)) {
      parentRenderable = objectData.__parentRenderable.getObjectData().__identifierPath + ' (filtered)';
      delete objectData.__parentRenderable;
    }

    // @todo(bnf): avoid any
    //const myObjectData: Omit<T, 'renderables' | '__parentRenderable'> & { renderables: object[], __parentRenderable: string} = objectData as unknown;
    const myObjectData: any = objectData;
...
function createModel<D extends object, T extends ModelMetadata<D>>(modelExtension: T): Model<D, T> {
...
export class Repository {
  ...
  public setFormEditorDefinitions(formEditorDefinitions: FormEditorDefinitions): void {
    ...
    for (const _key1 of Object.keys(formEditorDefinitions)) {
      ...
      for (const _key2 of Object.keys(formEditorDefinitions[key1])) {
        const key2 = _key2 as keyof keyof FormEditorDefinitions;
        if (
          formEditorDefinitions[key1][key2 as any] === null ||
          typeof formEditorDefinitions[key1][key2 as any] !== 'object'
        ) {
          formEditorDefinitions[key1][key2 as any] = {} as any;
        }
      }
    }
...
export class Factory {
  ...
  public createFormElement(
    ...
    const predefinedDefaults: any = formElementTypeDefinition.predefinedDefaults || {};
    for (const collectionName of Object.keys(configuration)) {
      // @todo(bnf) ugly access to repository
      ...
      collections[collectionName] = <Record<string, CollectionElementConfiguration>>$.extend(
        ...
        (configuration as any)[collectionName]
      );

      delete predefinedDefaults[collectionName];
      delete (configuration as any)[collectionName];
    }
...
export class DataBackend {
  ...
  public saveFormDefinition(): void {
    ...
    runningAjaxRequests.saveForm.fail(function(jqXHR, textStatus, errorThrown) {
  ...
  public renderFormDefinitionPage(pageIndex: number): void {
    ...
    runningAjaxRequests.renderFormDefinitionPage.fail(function(jqXHR, textStatus, errorThrown) {
      publisherSubscriber.publish('core/ajax/error', [jqXHR, textStatus, errorThrown]);
    });
  }
}
...
export class ApplicationStateStack {
  ...
  public incrementCurrentStackPointer() {
    this.setCurrentStackPointer(++this.stackPointer);
  }
}
