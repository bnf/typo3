...
A basic app shipped with the system extension can be used to create records
...
To create a new app navigate to the :guilabel:`System > Integrations > Hub` backend
module. If you call it the first time you will see the invitation to create a
new app:

..  figure:: /Images/BackendModuleEmpty.png
    :alt: Backend module "Hub" with no hub available
    :class: with-shadow

    Backend module "Hub" with no hub available
...
Click on the button :guilabel:`Create new app` to add a new app.
...
App Type
    Select one of the available app types. The system extension comes
    with the :guilabel:`Create database record`.

Name
    Give the app a meaningful name. The name is displayed on the overview
    page of the backend module.

Description
    You can provide additional information to describe, for example, the purpose
    of this app in detail.

Identifier
    Any app record is defined by a unique identifier. It is part of the
    TYPO3 URL when calling this app.

Secret
    The secret is necessary to authorize the app from the outside. It can
...
app type. For the "Create database record" the following fields are
available:
...
We now save and close the form - our newly created app is now visible:

..  figure:: /Images/BackendModuleWithRecord.png
    :alt: Backend module with app
    :class: with-shadow

    Backend module with app

..  _call-app:

Call the app manually
==========================

TYPO3 can now react on this app. Clicking on the :guilabel:`Example`
...
..  code-block:: bash
    ...
    curl -X 'POST' \
        'https://example.com/typo3/app/a5cffc58-b69a-42f6-9866-f93ec1ad9dc5' \
...
create record app. In case you want to allow your own tables to be
available in the app's table selection, add the table in a corresponding
:ref:`TCA override file <t3coreapi:storing-changes-extension-overrides>`:

..  code-block:: php
    :caption: EXT:my_extension/Configuration/TCA/Overrides/sys_app.php

    if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('hub')) {
...
In case your extension depends on EXT:hub the :php:`isLoaded()` check
