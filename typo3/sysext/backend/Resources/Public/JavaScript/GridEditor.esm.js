...
class GridEditor {
    ...
    constructor(config = null) {
        ...
        this.selectorCodeMirror = '.t3js-grideditor-preview-config .CodeMirror';
        /**
         *
         * @param {Event} e
         */
    ...
    writeConfig(data) {
        ...
        let content = 'mod.web_layout.BackendLayouts {\n' +
            ...
            '  }\n' +
            '}\n';
        jQuery(this.selectorConfigPreview).find(this.selectorPreviewArea).empty().append(content);
        // Update CodeMirror content if instantiated
        const codemirror = document.querySelector(this.selectorCodeMirror);
        if (codemirror) {
    ...
    export2LayoutRecord() {
        ...
        for (let row = 0; row < this.rowCount; row++) {
            ...
            for (let col = 0; col < this.colCount; col++) {
                ...
                if (cell) {
                    if (!cell.spanned) {
                        const cellName = GridEditor.stripMarkup(cell.name) || '';
                        colIndex++;
                        result += '\t\t\t\t' + (colIndex) + ' {\n';
