 ...
 */
let SelectTree = class SelectTree extends SvgTree {
    constructor() {
        super();
     ...
     */
    collapseAll() {
        this.nodes.forEach((node) => { this.hideChildren(node); });
        this.prepareDataForVisibleNodes();
        this.updateVisibleNodes();
    }
    /**
    ...
    renderCheckbox(nodeSelection) {
        ...
        const g = nodeSelection.filter((node) => {
            ...
            .on('click', (evt, node) => this.selectNode(node));
        g.append('use')
            .attr('x', 28)
            .attr('y', -8)
...
SelectTree = __decorate([
    n('typo3-backend-form-selecttree')
], SelectTree);
