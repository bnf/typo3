...
class DragUploaderPlugin {
    ...
    drawOverrideModal() {
        ...
        for (let i = 0; i < amountOfItems; ++i) {
            const $record = jQuery('<tr />').append(jQuery('<td />').append((this.askForOverride[i].original.thumbUrl !== ''
                ...
                __import_moment(this.askForOverride[i].uploaded.lastModified
                    ...
                    : this.askForOverride[i].uploaded.lastModifiedDate).format('YYYY-MM-DD HH:mm')), jQuery('<td />').append(jQuery('<select />', { class: 'form-select t3js-actions', 'data-override': i }).append((this.irreObjectUid ? jQuery('<option/>').val(Action.USE_EXISTING).text(TYPO3.lang['file_upload.actions.use_existing']) : ''), jQuery('<option />', { 'selected': this.defaultAction === Action.SKIP })
        ...
        $modal.find('.modal-footer').prepend(jQuery('<span/>').addClass('form-inline').append(jQuery('<label/>').text(TYPO3.lang['file_upload.actions.all.label']), jQuery('<select/>', { class: 'form-select t3js-actions-all' }).append(jQuery('<option/>').val('').text(TYPO3.lang['file_upload.actions.all.empty']), (this.irreObjectUid ? jQuery('<option/>').val(Action.USE_EXISTING).text(TYPO3.lang['file_upload.actions.all.use_existing']) : ''), jQuery('<option/>', { 'selected': this.defaultAction === Action.SKIP })
