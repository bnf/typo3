...
class r extends HTMLElement{constructor(){super(...arguments),this.element=null}connectedCallback(){const e=this.getAttribute("recordFieldId")
null!==e&&(this.element=this.querySelector(n`#${e}`),this.element&&this.registerEventHandler())}registerEventHandler(){const t=document.querySelector(n`input[name="${this.element.dataset.formengineInputName}"]`)
new e("blur",(e=>{t.value=e.target.value,this.handleEvent(e)})).bindTo(this.element),new e("formengine.cp.change",(e=>{this.handleEvent(e)})).bindTo(this.element)}handleEvent(e){t.validateField(e.target),t.markFieldAsChanged(e.target),document.querySelectorAll(".module-docheader-bar .btn").forEach((e=>{e.classList.remove("disabled"),e.disabled=!1}))}}window.customElements.define("typo3-formengine-element-color",r)
