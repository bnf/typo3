...
if(!0===l)if(!0===s[a]){if(!o){(i=document.querySelector('[data-bs-target="#'+a+'"]')).classList.remove("collapsed"),i.setAttribute("aria-expanded","true"),e.classList.add("show")}}else if(o){(i=document.querySelector('[data-bs-target="#'+a+'"]')).classList.add("collapsed"),i.setAttribute("aria-expanded","false"),e.classList.remove("show")}if(!0===r)if(!1===s[a]){if(o){(i=document.querySelector('[data-bs-target="#'+a+'"]')).classList.add("collapsed"),i.setAttribute("aria-expanded","false"),e.classList.remove("show")}}else if(!o){const i;(i=document.querySelector('[data-bs-target="#'+a+'"]')).classList.remove("collapsed"),i.setAttribute("aria-expanded","true"),e.classList.add("show")}}))}shallRecoverState(e){return void 0===e.dataset.persistCollapseStateNotIfSearch||"false"===e.dataset.persistCollapseStateNotIfSearch||null!==this.searchField&&""===this.searchField.value}fromStorage(t){let s
