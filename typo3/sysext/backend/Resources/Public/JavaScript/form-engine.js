...
return n&&("db"===e?o.expandPage=n:o.expandFolder=n),a.advanced({type:a.types.iframe,content:g.browserUrl+"&"+new URLSearchParams(o).toString(),size:a.sizes.large})},g.setSelectOptionFromExternalSource=function(e,n,a,o,exclusiveValues=[],w=void 0){let i,r,l=!1,c=!1
i=(r=g.getFieldElement(e)).get(0)
const d=r.get(0)
if(null===d||"--div--"===n||d instanceof HTMLOptGroupElement)return
const u=g.getFieldElement(e,"_list",!0)
if(u.length>0&&(i=(r=u).get(0),l=r.prop("multiple")&&"1"!=r.prop("size"),c=!0),l||c){const m=g.getFieldElement(e,"_avail"),f=m.get(0)
if(!l){for(const p of i.querySelectorAll("option")){(k=m.find(s`option[value="${t(p).attr("value")}"]`)).length&&(k.removeClass("hidden").prop("disabled",!1),g.enableOptGroup(k.get(0)))}r.empty()}if(exclusiveValues.length>0){let h=!1;(exclusiveValues.includes(n)||1==r.find("option").length&&exclusiveValues.includes(r.find("option").prop("value")))&&(r.empty(),h=!0),h&&void 0!==w&&w.closest("select").querySelectorAll("[disabled]").forEach((function(e){e.classList.remove("hidden"),e.disabled=!1,g.enableOptGroup(e)}))}let v=!0
const b=g.getFieldElement(e,"_mul",!0)
if(0==b.length||0==b.val()){for(const w of i.querySelectorAll("option"))if(w.value==n){v=!1
break}if(v&&void 0!==w){w.classList.add("hidden"),w.disabled=!0
const y=w.parentElement
y instanceof HTMLOptGroupElement&&0===y.querySelectorAll("option:not([disabled]):not([hidden]):not(.hidden)").length&&(y.disabled=!0,y.classList.add("hidden"))}}if(v){const k;(k=t("<option></option>")).attr({value:n,title:o}).text(a),k.appendTo(r),g.updateHiddenFieldValueFromSelect(i,d),g.Validation.markFieldAsChanged(d),g.Validation.validateField(i),g.Validation.validateField(f)}}else{const E=n.toString().match(/_(\d+)$/)
null!=E&&(n=E[1]),r.val(n),g.Validation.validateField(i)}},g.updateHiddenFieldValueFromSelect=function(e,t){const n=Array.from(e.options).map((e=>e.value))
...
if("0"===t.closeDoc?.value&&null!==e.submitter&&("A"===e.submitter.tagName||e.submitter.hasAttribute("form"))&&!e.defaultPrevented){const n=t.querySelector(s`input[name="${g.doSaveFieldName}"]`)
null!==n&&(n.value="1")}})),window.addEventListener("message",g.handlePostMessage)},g.consume=function(e){if(!e)throw new r.BackendException("No interaction request given",1496589980)
let t
const n=new Promise(((e,n)=>{t={resolve:e,reject:n}}))
if(e.concernsTypes(g.consumeTypes)){const a=e.outerMostRequest
g.interactionRequestMap.attachFor(a,t),a.isProcessed()?f(a,a.getProcessedData().response):g.hasChange()||g.isNew()?g.preventExitIfNotSaved((function(e){a.setProcessedData({response:e}),f(a,e)})):g.interactionRequestMap.resolveFor(a)}return n},g.handlePostMessage=function(e){if(!o.MessageUtility.verifyOrigin(e.origin))throw"Denied message sent by "+e.origin
...
else{(a=document.createElement("div")).classList.add("t3js-charcounter-min")
const i=document.createElement("span")
i.classList.add("badge","badge-danger"),i.innerHTML=o,a.append(i)
let r=n.querySelector(".t3js-charcounter-wrapper")
r||((r=document.createElement("div")).classList.add("t3js-charcounter-wrapper"),n.append(r)),r.prepend(a)}},t=e=>{const t=e.currentTarget.closest(".t3js-formengine-field-item").querySelector(".t3js-charcounter-min")
t&&t.remove()}
...
return 0===a?0:n-a-(t.match(/\n/g)||[]).length},g.initializeNullNoPlaceholderCheckboxes=function(){document.querySelectorAll(".t3-form-field-eval-null-checkbox").forEach((e=>{const t=e.querySelector('input[type="checkbox"]'),n=e.closest(".t3js-formengine-field-item")
t.checked||n.classList.add("disabled")}))},g.initializeNullWithPlaceholderCheckboxes=function(){document.querySelectorAll(".t3js-form-field-eval-null-placeholder-checkbox").forEach((e=>{g.toggleCheckboxField(e.querySelector('input[type="checkbox"]'),!1)}))},g.toggleCheckboxField=function(e,triggerFocusWhenChecked=!0){const t=e.closest(".t3js-formengine-field-item"),n=t.querySelector(".t3js-formengine-placeholder-placeholder"),a=t.querySelector(".t3js-formengine-placeholder-formfield")
e.checked?(n.hidden=!0,a.hidden=!1,triggerFocusWhenChecked&&a.querySelector("input,select,textarea")?.focus()):(n.hidden=!1,a.hidden=!0)},g.reinitialize=function(){const e=document.querySelectorAll(".t3js-clearable")
...
g.hasChange()||g.isNew()?g.showPreviewModal(a,o,i,n):(t(s`form[name="${g.formName}"]`).append(i),window.open("","newTYPO3frontendWindow"),g.formElement.submit())},g.previewActionCallback=function(e,n,o){switch(a.dismiss(),e){case"discard":const i=window.open(n,"newTYPO3frontendWindow")
i.focus(),c.urlsPointToSameServerSideResource(i.location.href,n)&&i.location.reload()
break
...
l=e?[c,d]:[c,s,d],a.confirm(o,r,i.info,l).addEventListener("button.clicked",(function(e){n(e.target.name,t)}))},g.duplicateAction=function(e,n){n=n||g.duplicateActionCallback
...
l=e?[c,d]:[c,s,d],a.confirm(o,r,i.info,l).addEventListener("button.clicked",(function(e){n(e.target.name,t)}))},g.deleteAction=function(e,n){n=n||g.deleteActionCallback
const a=t(e.target)
