...
for(const e of this.invalidFields){const i=e.closest(".t3js-formengine-validation-marker")
if(null===i)throw console.error(e),new Error("Could not find an element containing the `t3js-formengine-validation-marker` class for the previously logged input field.")
const o=i.querySelector("[data-formengine-validation-rules]")
if(null===o)throw console.error(i),new Error("Could not find an element containing the `data-formengine-validation-rules` attribute for the previously logged container.")
const n=document.createElement("a")
n.classList.add("list-group-item"),n.href="#",n.textContent=i.querySelector(this.labelSelector)?.textContent||"",n.addEventListener("click",(e=>{this.switchToField(e,i,o)})),t.append(n)}e.classList.remove("hidden"),i.setOptions(e,{html:!0,content:t})}else e.classList.add("hidden"),i.hide(e)}switchToField(e,i,l){e.preventDefault()
let r=l
for(;r;){if(r.matches('[id][role="tabpanel"]')){const e=document.querySelector(t`[aria-controls="${r.id}"]`)
new o(e).show()}r=r.parentElement}l.checkVisibility()?l.focus():n.scrollIntoViewIfNeeded(i)}}