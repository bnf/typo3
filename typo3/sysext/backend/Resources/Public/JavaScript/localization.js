...
else{const p=document.createElement("div")
p.dataset.bsToggle="buttons",p.append(...d.map((e=>document.createRange().createContextualFragment(e)))),c.addSlide("localize-choose-action",TYPO3.lang["localize.wizard.header_page"].replace("{0}",i.data("page")).replace("{1}",i.data("languageName")),p,a.notice,TYPO3.lang["localize.wizard.step.selectMode"],((e,t)=>{void 0!==t.localizationMode&&c.unlockNextStep()}))}1===g.length?c.set("sourceLanguage",g[0].uid):c.addSlide("localize-choose-language",TYPO3.lang["localize.view.chooseLanguage"],"",a.notice,TYPO3.lang["localize.wizard.step.chooseLanguage"],(async(e,a)=>{void 0!==a.sourceLanguage&&c.unlockNextStep(),e.html('<div class="text-center">'+await n.getIcon("spinner-circle",n.sizes.large)+"</div>"),c.getComponent().on("change",".t3js-language-option",(e=>{c.set("sourceLanguage",t(e.currentTarget).val()),c.unlockNextStep()}))
const o=t("<div />",{class:"row"})
for(const l of g){const s="language"+l.uid,i=t("<input />",{type:"radio",name:"language",id:s,value:l.uid,class:"btn-check t3js-language-option"}),d=t("<label />",{class:"btn btn-default btn-block",for:s}).text(" "+l.title).prepend(l.flagIcon)
o.append(t("<div />",{class:"col-sm-4"}).append(i).append(d))}e.empty().append(o)})),c.addSlide("localize-summary",TYPO3.lang["localize.view.summary"],"",a.notice,TYPO3.lang["localize.wizard.step.selectRecords"],(async(e,a)=>{e.empty().html('<div class="text-center">'+await n.getIcon("spinner-circle",n.sizes.large)+"</div>")
...
else{const i=a.records.indexOf(n)
i>-1&&a.records.splice(i,1)}const d=l.find(".t3js-localization-toggle-record"),r=l.find(".t3js-localization-toggle-record:checked")
s.prop("checked",r.length>0),s.prop("__indeterminate",r.length>0&&r.length<d.length),a.records.length>0?c.unlockNextStep():c.lockNextStep()})).on("change",".t3js-localization-toggle-column",(e=>{const a=t(e.currentTarget),o=a.closest("fieldset").find(".t3js-localization-toggle-record")
