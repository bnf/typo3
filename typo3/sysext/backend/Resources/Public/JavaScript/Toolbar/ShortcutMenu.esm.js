            ...
            Modal.confirm(TYPO3.lang['bookmark.create'], confirmationText).on('confirm.button.ok', (e) => {
                ...
                (new AjaxRequest(TYPO3.settings.ajaxUrls.shortcut_create)).post({
                    ...
                    if (typeof shortcutButton === 'object') {
                        const isDropdownItem = jQuery(shortcutButton).hasClass('dropdown-item');
                        const securityUtility = new SecurityUtility();
                        Icons.getIcon('actions-system-shortcut-active', Icons.sizes.small).then((icon) => {
                            jQuery(shortcutButton).html(icon + (isDropdownItem ? ' ' + securityUtility.encodeHtml(TYPO3.lang['labels.alreadyBookmarked']) : ''));
                        });
                        jQuery(shortcutButton).addClass(isDropdownItem ? 'disabled' : 'active');
                        jQuery(shortcutButton).attr('onclick', null);
                        jQuery(shortcutButton).attr('title', TYPO3.lang['labels.alreadyBookmarked']);
                    }
                });
