...
let x=class extends t{constructor(){super(...arguments),this.mode=null,this.addons=[],this.keymaps=[],this.lineDigits=0,this.autoheight=!1,this.nolazyload=!1,this.readonly=!1,this.fullscreen=!1,this.panel="bottom",this.editorTheme=null,this.editorView=null}static{this.styles=r`:host{display:block;position:relative}:host([fullscreen]){inset:64px 0 0;position:fixed;z-index:9}:host([fullscreen]) .cm-scroller{max-height:100%;min-height:auto}:host([autoheight]) .cm-scroller{max-height:none}.codemirror-label{font-size:.875em;opacity:.75}.codemirror-label-top{margin-bottom:.25rem}.codemirror-label-bottom{margin-top:.25rem}typo3-backend-spinner{left:50%;position:absolute;top:50%;transform:translate(-50%,-50%)}.cm-editor{border:var(--typo3-input-border-width) solid var(--typo3-input-border-color);border-radius:var(--typo3-input-border-radius);overflow:hidden;transition:outline-color .15s ease-in-out,box-shadow .15s ease-in-out}.cm-focused{border-color:var(--typo3-input-focus-border-color);outline:.25rem solid color-mix(in srgb,var(--typo3-form-control-focus-border-color),transparent 25%);outline-offset:0}.cm-gutters{height:auto!important;position:relative!important}.cm-content{min-height:calc(8px + 16.8px*var(--rows, 18))!important}.cm-scroller{max-height:calc(100dvh - 10rem);min-height:100%}`}setContent(e){null!==this.editorView&&this.editorView.dispatch({changes:{from:0,to:this.editorView.state.doc.length,insert:e}})}getContent(){return this.editorView.state.doc.toString()}render(){return o`${this.label&&"top"===this.panel?o`<div class="codemirror-label codemirror-label-top">${this.label}</div>`:""}<div id="codemirror-parent" @keydown="${e=>this.onKeydown(e)}"></div>${this.label&&"bottom"===this.panel?o`<div class="codemirror-label codemirror-label-bottom">${this.label}</div>`:""} ${null===this.editorView?o`<typo3-backend-spinner size="large"></typo3-backend-spinner>`:""}`}firstUpdated(){if(this.nolazyload)return void this.initializeEditor(this.firstElementChild)
