...
return await t.resolve()}))}fetchFromRemote(t,o,n){return this.promiseCache.get(o,(async n=>{const s=await new i(TYPO3.settings.ajaxUrls.icons).withQueryArguments({icon:JSON.stringify(t)}).get({signal:n}),r=await s.resolve()
return!s.response.redirected&&r.startsWith("<span")&&r.includes("t3js-icon")&&r.includes('<span class="icon-markup">')&&e.set("icon_"+o,r),r}),n)}fetchFromLocal(i){return e.isset("icon_"+i)?Promise.resolve(e.get("icon_"+i)):Promise.reject()}}let a
