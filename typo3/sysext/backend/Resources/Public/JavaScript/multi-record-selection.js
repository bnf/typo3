...
class r{static{this.activeClass="active"}constructor(){this.lastChecked=null,t.ready().then((()=>{r.restoreTemporaryState(),this.registerActions(),this.registerActionsEventHandlers(),this.registerCheckboxActions(),this.registerCheckboxKeyboardActions(),this.registerCheckboxTableRowSelectionAction(),this.registerToggleCheckboxActions(),this.registerDispatchCheckboxStateChangedEvent(),this.registerCheckboxStateChangedEventHandler()}))}static getCheckboxes(e=l.any,t=""){return document.querySelectorAll(r.getCombinedSelector(MultiRecordSelectionSelectors.checkboxSelector+e,t))}static getCombinedSelector(e,t){return""!==t?[o`[data-multi-record-selection-identifier="${t}"]`,e].join(" "):e}static getIdentifier(e){return e.closest("[data-multi-record-selection-identifier]")?.dataset.multiRecordSelectionIdentifier||""}static changeCheckboxState(e,t){e.disabled||e.checked===t||e.dataset.manuallyChanged||(e.checked=t,e.dispatchEvent(new CustomEvent("change",{bubbles:!0})),e.dispatchEvent(new CustomEvent("multiRecordSelection:checkbox:state:changed",{detail:{identifier:r.getIdentifier(e)},bubbles:!0,cancelable:!1})))}static restoreTemporaryState(){const e=r.getCheckboxes(l.checked)
if(!e.length)return
let t=!1
const c=[]
e.forEach((e=>{e.closest(MultiRecordSelectionSelectors.elementSelector)?.classList.add(r.activeClass)
const o=r.getIdentifier(e)
""===o||c.includes(o)||(c.push(o),t=!0,r.toggleActionsState(o))})),t||r.toggleActionsState()}static toggleActionsState(e=""){const t=document.querySelectorAll(r.getCombinedSelector(MultiRecordSelectionSelectors.actionsSelector,e))
if(!t.length)return
if(!r.getCheckboxes(l.checked,e).length)return void t.forEach((e=>r.changeActionContainerVisibility(e,!1)))
t.forEach((e=>r.changeActionContainerVisibility(e)))
const c=document.querySelectorAll([r.getCombinedSelector(MultiRecordSelectionSelectors.actionsSelector,e),i.actionButton].join(" "))
c.length&&c.forEach((t=>{if(!t.dataset.multiRecordSelectionActionConfig)return
const c=JSON.parse(t.dataset.multiRecordSelectionActionConfig)
if(!c.idField)return
t.disabled=!0
const o=r.getCheckboxes(l.checked,e)
for(let e=0;e<o.length;e++)if(o[e].closest(MultiRecordSelectionSelectors.elementSelector)?.dataset[c.idField]){t.disabled=!1
break}}))}static changeActionContainerVisibility(e,t=!0){const c=e.closest(".multi-record-selection-panel")?.children
if(t){if(c)for(let e=0;e<c.length;e++)c[e].classList.add("hidden")
e.classList.remove("hidden")}else{if(c)for(let e=0;e<c.length;e++)c[e].classList.remove("hidden")
...
null!==s&&(s.disabled=!r.getCheckboxes(l.any,c).length)})).delegateTo(document,MultiRecordSelectionSelectors.checkboxActionsToggleSelector)}handleCheckboxKeyboardActions(e,t,c=!0){const o=r.getIdentifier(t)
if(this.lastChecked&&document.body.contains(this.lastChecked)&&r.getIdentifier(this.lastChecked)===o&&(e.shiftKey||e.altKey||e.ctrlKey)){if(c&&r.unsetManuallyChangedAttribute(o),e.shiftKey){const e=Array.from(r.getCheckboxes(l.any,o)),c=e.indexOf(t),i=e.indexOf(this.lastChecked)
e.slice(Math.min(c,i),Math.max(c,i)+1).forEach((e=>{e!==t&&r.changeCheckboxState(e,t.checked)}))}this.lastChecked=t,(e.altKey||e.ctrlKey)&&r.getCheckboxes(l.any,o).forEach((e=>{e!==t&&r.changeCheckboxState(e,!e.checked)})),r.unsetManuallyChangedAttribute(o)}else this.lastChecked=t}}export default new r
