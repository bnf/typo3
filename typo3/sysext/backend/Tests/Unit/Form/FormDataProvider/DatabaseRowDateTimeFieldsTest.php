...
final class DatabaseRowDateTimeFieldsTest extends UnitTestCase
{
    ...
    public function addDataConvertsTimeStringToTimestamp(): void
    {
        ...
        $expected = $input;
        // @todo should actually be 1970
        $expected['databaseRow']['aField'] = new \DateTimeImmutable(date('Y-m-d') . 'T15:25:32', new \DateTimeZone('UTC'));
        self::assertEquals($expected, (new DatabaseRowDateTimeFields())->addData($input));
        date_default_timezone_set($oldTimezone);
    }
    ...
    public function addDataConvertsMidnightTimeStringOfDefaultFieldToDefaultValue(): void
    {
        ...
        $expected = $input;
        $expected['databaseRow']['aField'] = new \DateTimeImmutable(date('Y-m-d') . 'T00:00:00+00:00', new \DateTimeZone('UTC'));
    ...
    #[Test]
    public function addDataConvertsMidnightTimeStringOfExplicitNullableFieldToMidnightDateTimeObject(): void
    {
        $oldTimezone = date_default_timezone_get();
        date_default_timezone_set('UTC');
        $input = [
        ...
        $expected = $input;
        $expected['databaseRow']['aField'] = new \DateTimeImmutable(date('Y-m-d') . 'T00:00:00', new \DateTimeZone('UTC'));
    ...
    #[Test]
    public function addDataResolvesNullValueForEmptyDateValue(): void
    {
    ...
    /* @todo this test needs to be moved into the persistence layer, it is not longer a parser topic
    #[Test]
