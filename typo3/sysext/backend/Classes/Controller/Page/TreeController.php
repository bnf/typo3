...
class TreeController
{
    ...
    protected function getAllEntryPointPageTrees(int $startPid = 0, string $query = ''): array
    {
        ...
        if ($entryPointIds === null) {
            ...
            // Switch to multiple-entryPoint-mode if the rootPage is to be
            // mounted. (pages would appear duplicated below rootPid otherwise)
        ...
        if ($entryPointIds === null) {
            ...
            $hiddenRecords = $this->hiddenRecords;
            $entryPointIds = array_filter($entryPointIds, function ($id) use ($hiddenRecords) {
                return !in_array($id, $hiddenRecords, true);
            });
