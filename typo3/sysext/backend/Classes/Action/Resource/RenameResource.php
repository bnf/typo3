    ...
    public function perform(
        ...
        try {
            ...
            if ($oldName === $resourceName) {
                $message = sprintf($context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.error.message.resourceNameNotDifferent'), $oldName);
                return [
                    'message' => $message,
            ...
            if ($resource->getName() === $oldName) {
                $message = sprintf($context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.error.message.resourceNotRenamed'), $oldName);
            ...
            throw new ActionException($this->renderResultException($exception, $context->languageService), 1768337411, $exception);
        } catch (\Exception $exception) {
            $message = match ($exception->getCode()) {
                1676979120 => $context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.error.message.resourceNotFileOrFolder'),
                1676299579 => $context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.error.message.resourceOutsideOfStorages'),
                1676979130 => $context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.error.message.resourceNoPermissionRename'),
                1676978732 => $context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.error.message.resourceNameCannotBeEmpty'),
        ...
        return [
            'message' => sprintf(
                $context->languageService->sL('LLL:EXT:backend/Resources/Private/Language/locallang_resource.xlf:ajax.success.message.renamed'),
                $oldName,
                $resource->getName()
            ),
            'origin' => $this->getResourceData($origin),
