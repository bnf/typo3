...
class PageTreeRepository
{
    ...
    public function getTreeLevels(array $pageTree, int $depth, array $entryPointIds = []): array
    {
        ...
        if (count($entryPointIds) > 0) {
            ...
            // Avoid groupAndSortPages() and simply preserve ordering of the
            // $entryPointIds array. DB mounts aren't sorted against each other,
            // that means sorting values are not meaningful).
            $groupedAndSortedPagesByPid[0] = array_values($pageRecords);
