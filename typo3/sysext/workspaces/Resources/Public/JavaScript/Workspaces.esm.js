...
class Workspaces {
    ...
    renderSendToStageWindow(response) {
        ...
        if (typeof result.sendMailTo !== 'undefined' && result.sendMailTo.length > 0) {
            ...
            $form.append($('<div />', { class: 'form-group' }).append($('<button type="button" class="btn btn-default btn-xs t3js-workspace-recipients-selectall" />')
                .text(TYPO3.lang['window.sendToNextStageWindow.selectAll']), '&nbsp;', $('<button type="button" class="btn btn-default btn-xs t3js-workspace-recipients-deselectall" />')
