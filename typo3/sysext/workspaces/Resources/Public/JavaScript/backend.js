...
else{const s=this.markedRecordsForMassAction.indexOf(o)
s>-1&&this.markedRecordsForMassAction.splice(s,1)}n.dataset.collectionCurrent?m.changeCollectionChildrenState(n.dataset.collectionCurrent,a):n.dataset.collection&&(m.changeCollectionChildrenState(n.dataset.collection,a),m.changeCollectionParentState(n.dataset.collection,a)),document.querySelector(u.chooseMassAction).disabled=this.markedRecordsForMassAction.length>0},this.openIntegrityWarningModal=()=>{const e=o.confirm(TYPO3.lang["window.integrity_warning.title"],t`<p>${TYPO3.lang["integrity.hasIssuesDescription"]}<br>${TYPO3.lang["integrity.hasIssuesQuestion"]}</p>`,n.warning)
...
a.addEventListener("button.clicked",(t=>{if("ok"===t.target.name){const s=i.convertFormToObject(a.querySelector("form"))
s.affects={table:e.dataset.table,nextStage:n,t3ver_oid:e.dataset.t3ver_oid,uid:e.dataset.uid,elements:[]},this.sendRemoteRequest([this.generateRemoteActionsPayload(o,[s]),this.generateRemotePayload("getWorkspaceInfos",this.settings)]).then((async e=>{const t=await e.resolve()
...
this.resetMassActionState(e.data.length),this.buildPagination(e.total),0===e.total?(t.style.display="none",n.style.display="block"):(t.style.display="block",n.style.display="none"),document.querySelector("typo3-workspaces-record-table").results=e.data}buildPagination(e){const t=document.querySelector(u.pagination)
...
for(let o=0;o<this.markedRecordsForMassAction.length;++o){const s=this.markedRecordsForMassAction[o].split(":")
n.push({table:s[0],uid:s[1],t3ver_oid:s[2]})}this.sendRemoteRequest(this.generateRemoteActionsPayload("sendToSpecificStageWindow",[a,n])).then((async e=>{const t=this.renderSendToStageWindow(await e.resolve())
t.addEventListener("button.clicked",(e=>{if("ok"===e.target.name){const o=i.convertFormToObject(t.querySelector("form"))
o.affects={elements:n,nextStage:a},this.sendRemoteRequest([this.generateRemoteActionsPayload("sendToSpecificStageExecute",[o]),this.generateRemotePayload("getWorkspaceInfos",this.settings)]).then((async e=>{const n=await e.resolve()
...
for(const[language,url]of Object.entries(t)){const s=document.createElement("dt")
s.textContent=language
const i=document.createElement("a")
i.href=url,i.target="_blank",i.textContent=url
const c=document.createElement("dd")
c.appendChild(i),a.append(s,c)}o.show(TYPO3.lang.previewLink,a,n.info,[{text:TYPO3.lang.ok,active:!0,btnClass:"btn-info",name:"ok",trigger:(e,t)=>t.hideModal()}],["modal-inner-scroll"])}))}resetMassActionState(e){if(this.markedRecordsForMassAction=[],e){document.querySelector(u.workspaceActions).classList.remove("hidden"),document.querySelector(u.chooseMassAction).disabled=!1}document.dispatchEvent(new CustomEvent("multiRecordSelection:actions:hide"))}}export default new m
