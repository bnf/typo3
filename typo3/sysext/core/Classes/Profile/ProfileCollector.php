...
class ProfileCollector
{
    ...
    public function addProfileFromYaml(\SplFileInfo $fileInfo): void
    {
        ...
        if (is_file($settingsDefinitionsFile)) {
            $settingsDefinitions = $this->yamlFileLoader->load($settingsDefinitionsFile, 0, true);
            $version = (int)($settingsDefinitions['version'] ?? 0);
            if ($version !== 1) {
                throw new \RuntimeException('Settings definitions schema `version: 1` expected. Filename: ' . $settingsDefinitionsFile, 1711024378);
            }
            unset($settingsDefinitions['version']);
            if (!is_array($settingsDefinitions['settings'] ?? null)) {
                throw new \RuntimeException('Missing "settings" key in settings definitions. Filename: ' . $settingsDefinitionsFile, 1711024379);
            }
