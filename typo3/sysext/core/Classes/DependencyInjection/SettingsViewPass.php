...
final class SettingsViewPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->hasDefinition(SettingsDefinitionsCollection::class)) {
            return;
        }
        $settingsDefinitionsCollectionDefinition = $container->findDefinition(SettingsDefinitionsCollection::class);
        ...
        foreach ($container->findTaggedServiceIds($this->tagName) as $serviceName => $tags) {
            ...
            foreach ($tags as $attributes) {
                ...
                $settingsServiceDefinition->setArguments([
                    // @todo decide whether to support more than a "system" type
                    //$type,
                    $namespace,
                    $settingsServiceDefinition->getClass(),
                ]);

                $this->parseProperties($settingsServiceDefinition->getClass(), $settingsDefinitionsCollectionDefinition);
            }
        }
    }
