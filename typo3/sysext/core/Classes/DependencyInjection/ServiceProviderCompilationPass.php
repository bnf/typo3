...
class ServiceProviderCompilationPass implements CompilerPassInterface
{
    ...
    private function registerService(ContainerBuilder $container, string $serviceName, string $serviceProviderKey, callable $callable): void
    {
    ...
    private function extendService(ContainerBuilder $container, string $serviceName, string $serviceProviderKey, callable $callable): void
    {
        ...
        if ($container->has($serviceName)) {
            list($finalServiceName, $previousServiceName) = $this->getDecoratedServiceName($container, $serviceName);
