...
class MonologLoggerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds('monolog.handler') as $id => $tags) {
            ...
            foreach ($tags as $tag) {
                $handlerServices[] = [
                    ...
                    'excluded_channels' => explode(',', $tag['channels'] ?? ''),
                ];
            }
        }
        ...
        $handlerFactoryRegistry->setArguments([new Reference('service_container'), $handlerFactories, $handlerProcessors, $handlerServices]);
