...
use Bnf\SymfonyServiceProviderCompilerPass\Registry as ServiceProviderRegistry;
...
class ContainerBuilder
{
     ...
     * Constructor
     */
    public function __construct(array $defaultServices)
    {
        // We provide static parameters as services, to be usable by service providers.
        $this->defaultServices = $defaultServices + $this->getStaticParameters() + [ self::class => $this ];
    ...
    {
        $registry = new ServiceProviderRegistry($this->getServiceProviders($packageManager));
    ...
    {
        $serviceProviders = $this->getServiceProviders($packageManager, $failsafe);

        if ($failsafe) {
            return new FailsafeContainer($serviceProviders, $this->defaultServices);
        }
        ...
        $registry = new ServiceProviderRegistry($serviceProviders);
        ...
        $container->set($this->serviceProviderRegistryServiceName, $registry);
    ...
    protected function dumpContainer(SymfonyContainerBuilder $containerBuilder, FrontendInterface $cache): string
    {
        ...
        // instead of `new`
        // @todo: find a way to replace all `new`s with GenralUtility::makeInstance in symfony directly
        //        …or maybe leave this here until GeneralUtility::makeInstance() is dropped
     ...
     * @internal
     */
    public function getServiceProviders(PackageManager $packageManager, bool $failsafe = false, bool $lazy = null): array
    {
        $serviceProviders = [];
        $lazy = $lazy ?? !$failsafe;
    ...
    {
        // @todo: also hash the defaultServices into the cache identifier? e.g. using something like: implode(',', array_keys($this->defaultServices))
