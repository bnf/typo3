...
final class LoggerAwarePass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        foreach ($container->findTaggedServiceIds($this->tagName) as $id => $tags) {
            ...
            $channel = $id;
            if ($definition->isAutoconfigured() && $definition->getClass()) {
                if ($reflectionClass = $container->getReflectionClass($definition->getClass(), false)) {
                    $channel = $this->getChannelName($reflectionClass) ?? $channel;
                }
            }
    ...
    protected function getChannelName(\ReflectionClass $class): ?string
    {
        ...
        if ($class->getParentClass() !== false) {
            return $this->getChannelName($class->getParentClass());
        }
