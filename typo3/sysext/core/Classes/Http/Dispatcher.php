...
class Dispatcher implements DispatcherInterface
{
    ...
    protected function getCallableFromTarget($target)
    {
        ...
        if (is_string($target) && strpos($target, ':') === false) {
            ...
            // todo: switch to get() once all modules are stateless
            //$targetObject = $this->container->make($target);
            // get() is a bit of a risk here. requires the class
            // to be stateless in case it is dispatched multiple
            // times
            $targetObject = $this->container->get($target);
        ...
        if (is_string($target) && strpos($target, '::') !== false) {
            ...
            // todo: switch to get() once all modules are stateless
            //$targetObject = $this->container->make($className);
            // get() is a bit of a risk here. requires the class
            // to be stateless in case it is dispatched multiple
            // times
            $targetObject = $this->container->get($className);
