...
final readonly class Hydrator
{
    ...
    private function mapEnum(int|string $data, ResolvingSchema $schema): \BackedEnum|int|string
    {
    ...
    private function mapString(mixed $data, ResolvingSchema $schema, bool $coerce): string
    {
        // @todo datetime
        ...
        foreach ($values as $key => $value) {
            $values[$key] = array_key_exists($key, $schema->properties ?? []) ?
                $this->hydrate($value, $schema->properties[$key]) : (
                    $schema->additionalProperties !== null && $schema->additionalProperties instanceof Schema ?
