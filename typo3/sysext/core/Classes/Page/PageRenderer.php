    ...
    protected function computeRequireJsConfig($isDevelopment, array $packages)
    {
        ...
        foreach ($packages as $packageName => $package) {
            ...
            if (!empty($fullJsPath) && file_exists($absoluteJsPath)) {
                ...
                if ($package->getValueFromComposerManifest('type') !== 'typo3-cms-framework') {
                    $thirdPartyExtensionVersions[] = $package->getPackageKey() . ':' . $package->getPackageMetadata()->getVersion();
                }
            }
        }
        ...
        if ($isDevelopment) {
            ...
            $requireJsConfig['public']['urlArgs'] = 'bust=' . GeneralUtility::hmac(
                TYPO3_version . Environment::getProjectPath() . implode('|', $thirdPartyExtensionVersions)
            );
        }
