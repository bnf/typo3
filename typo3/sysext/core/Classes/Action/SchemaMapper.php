...
final readonly class SchemaMapper
{
    ...
    private function mapObject(ObjectType $type): array
    {
        ...
        foreach ($properties as $property => $reflection) {
            $fprops[$property] = $this->doMap($typeResolver->resolve($reflection));
        }
        ...
        foreach ($props as $property => $className) {
            $fprops[$property] = $this->doMap($extractor->getType($className, $property) ?? Type::null());
        }
        */
    ...
    private function mapGeneric(GenericType $type): array
    {
        ...
        return $this->doMap($type->getWrappedType());
    }

    private function mapTemplate(TemplateType $type): array
    {
        return $this->doMap($type->getBound());
    }
