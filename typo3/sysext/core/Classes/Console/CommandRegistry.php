...
class CommandRegistry implements CommandLoaderInterface, \IteratorAggregate, SingletonInterface
{
    ...
    /**
     * @return \Generator
     * @internal This will be removed in TYPO3 v11 when support for Configuration/Commands.php is dropped.
     */
    public function getNonLazyCommands(): \Generator
    {
        $this->populateCommandsFromPackages();
        foreach ($this->commands as $commandName => $command) {
    ...
    protected function populateCommandsFromPackages()
    {
        ...
        foreach ($this->lazyCommandConfigurations as $commandName => $commandConfig) {
            // Lazy commands shall be loaded from the Container commands shall be lazy loaded, store as string to indicate lazy loading
                ...
                if (is_array($commands)) {
                    foreach ($commands as $commandName => $commandConfig) {
                        if (array_key_exists($commandName, $this->lazyCommandConfigurations)) {
                            ...
                            // Skip this to allow extensions to provide commands per DI config and additionally in
                            // Configuration/Commands.php for TYPO3 v9 backwards compatbility.
                            // Note: Also the deprecation error is dropped on-demand as the extension has been
