                ...
                foreach ($subcategories as $subcategory) {
                    foreach ($subcategory['items'] as $setting) {
                        ...
                        switch ($type) {
                            case 'int':
                                $default = (int)$default;
                                if (isset($setting['typeIntMin'])) {
                                    $options['min'] = (int)$setting['typeIntMin'];
                                }
                                if (isset($setting['typeIntMax'])) {
                                    $options['max'] = (int)$setting['typeIntMax'];
                                }
                                break;
                            case 'int+':
                                $type = 'int';
                                $default = (int)$default;
                                $options = [
                                    'min' => (int)($setting['typeIntPlusMin'] ?? 0),
                                ];
                                if (isset($setting['typeIntPlusMax'])) {
                                    $options['max'] = (int)$setting['typeIntPlusMax'];
                                }
                                break;
                            case 'small':
                                $type = 'string';
                            ...
                            default:
                                $type = 'mixed';
                                if (is_string($default) && json_validate($default)) {
                                    $default = json_decode($default, false, 512, JSON_THROW_ON_ERROR);
                                }
                                break;
                        }
