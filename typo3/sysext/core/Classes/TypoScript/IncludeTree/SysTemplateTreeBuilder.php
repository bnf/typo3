...
final class SysTemplateTreeBuilder
{
    ...
    public function getTreeBySysTemplateRowsAndSite(
        ...
        $siteProfiles = $site instanceof Site ? $site->getProfiles() : [];
        if ($siteTypoScript !== null || count($siteProfiles) > 0) {
            $includeNode = null;
            ...
            if (!$includeNode) {
                ...
                $profiles = $this->profileRegistry->getProfiles(...$siteProfiles);
                if (count($profiles) > 0) {
        ...
        if (!$atLeastOneSysTemplateRowHasClearFlag && $siteTypoScript === null) {
