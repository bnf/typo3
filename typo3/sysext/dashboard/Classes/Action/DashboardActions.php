...
final readonly class DashboardActions
{
    ...
    public function updateDashboard(
        string $dashboardIdentifier,
        array $widgets,
        // @todo nested array is not properly deserialized and decoded as stdClass instead of array
    ...
    public function getWidgetSettings(
        ...
        $categories = [
            new Category(
                ...
                label: $context->translator->label($dashboardWidget->getTitle()),
                description: $context->translator->label($dashboardWidget->getDescription()),
                icon: $dashboardWidget->getIconIdentifier(),
                settings: array_map(
                    fn(SettingDefinition $definition): EditableSetting => new EditableSetting(
                        definition: $this->resolveSettingLabels($context->translator, $definition),
        ...
        foreach ($targetDashboard->getWidgets() as $widget) {
            ...
            if ($targetWidget->getIdentifier() === $widget->getIdentifier()) {
                ...
                if ($diff->changes === [] && $diff->deletions === []) {
                    return [
                        'status' => 'info',
                        'message' => $context->translator->label('LLL:EXT:dashboard/Resources/Private/Language/locallang.xlf:widget.settings.unchanged'),
                    ];
                }
    ...
    private function resolveSettingLabels(TranslatorInterface $translator, SettingDefinition $definition): SettingDefinition
    {
        return new SettingDefinition(...[
            ...get_object_vars($definition),
            'label' => $translator->label($definition->label),
            'description' => $definition->description !== null ? $translator->label($definition->description) : null,
            'enum' => array_map(static fn(string $label): string => $translator->label($label), $definition->enum),
        ]);
    }
