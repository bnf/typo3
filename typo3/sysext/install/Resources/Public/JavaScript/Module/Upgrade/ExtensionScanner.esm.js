...
import AjaxQueue from '../../Ajax/AjaxQueue.esm.js';
...
class ExtensionScanner extends AbstractInteractableModule {
    ...
    setProgressForAll() {
        ...
        if (numberOfScannedExtensions === numberOfExtensions) {
            ...
            NotificationService.success('Scan finished', 'All extensions have been scanned.');
            (new AjaxRequest(Router.getUrl())).post({
                ...
                const data = await response.resolve();
                if (data.success === true) {
                    NotificationService.success('Marked not affected files', 'Marked ' + data.markedAsNotAffected + ' ReST files as not affected.');
                }
            }, (error) => {
                Router.handleAjaxError(error, modalContent);
            });
    ...
    scanSingleExtension(extension) {
        ...
        (new AjaxRequest(Router.getUrl())).post({
            ...
            if (data.success === true && Array.isArray(data.files)) {
                ...
                if (numberOfFiles <= 0) {
                    NotificationService.warning('No files found', 'The extension ' + extension + ' contains no scannable files');
                    return;
                }
                ...
                NotificationService.error('Oops, an error occurred', 'Please look at the browser console output for details');
                console.error(data);
            }
        }, (error) => {
            Router.handleAjaxError(error, modalContent);
...
export default ExtensionScanner$1;
