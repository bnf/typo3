...
import{FileListDragDropEvent as n}from"@typo3/filelist/file-list-dragdrop.js"
var i
...
a.push(t)}))
const n={data:{[e]:a}}
new s(top.TYPO3.settings.ajaxUrls.file_process).post(n).then((async e=>{const t=await e.resolve()
this.handleMessages(t.messages??[]),o.ContentContainer.refresh(),top.document.dispatchEvent(new CustomEvent("typo3:filestoragetree:refresh"))})).catch((async e=>{const t=await e.resolve()
this.handleMessages(t.messages??[]),o.ContentContainer.refresh(),top.document.dispatchEvent(new CustomEvent("typo3:filestoragetree:refresh"))}))}handleMessages(e){e.forEach((e=>{a.showMessage(e.title||"",e.message||"",e.severity)}))}}
