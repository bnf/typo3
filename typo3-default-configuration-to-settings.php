...
class SchemaConverter
{
    ...
    private function parseElementList(object $schema, array $data, string $ns, array|string $default = null): object
    {
        $setting = $this->createSetting($ns, $data);
        $setting->type = 'stringlist';
        if (is_string($default)) {
    ...
    private function parseMap(object $schema, array $data, string $ns, array $default = null): object
    {
        $setting = $this->createSetting($ns, $data);
        $setting->type = 'stringmap';
        if ($default !== null) {
    ...
    private function createSetting(string $ns, array $data): object
    {
        ...
        $words = preg_split('/(?<=[a-z])(?=[A-Z])/', str_replace('_', ' ', str_replace('.', ': ', $title)));
    ...
    public function parseSchema(object $schema, array $data, string $ns, mixed $default = null, ?string $category = null): object
    {
        ...
        $object = match ($data['type']) {
            ...
            'element-list' => $this->parseElementList($schema, $data, $ns, $default),
            ...
            'map' => $this->parseMap($schema, $data, $ns, $default),
